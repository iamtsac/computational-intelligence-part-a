...
def train_model(x_train,y_train,x_test,y_test,epochs=5,nodes=128,verbose=0,loss='categorical_crossentropy',metric=['accuracy'],learning_rate=0.001,momentum=0,plot='off',r=0.0):

    # These variables are for the printing results at the end of training
    fold_number = 0 
    scores ,histories = list(), list()
    sum_of_loss = sum_of_acc =  0

    # initialiazing K-fold
...
for loss in loss_metrics:
    print("|----------------------------------------------------", "\n|  LOSS METRIC IS: ",loss," ")
    print('|----------------------------------------------------','\n|  Learning Rate:', default_learning_rate," Nodes:",opt_nodes,"")
    print('|----------------------------------------------------')

    history,plot = train_model(x_train,y_train,x_test,y_test,epochs=epochs,verbose=verbose,learning_rate=default_learning_rate,plot=plot,loss=loss,momentum=default_momentum,r=0)
